...
void cargar_catedra (catedra_t * catedra)
{
	...
	{	
		cargar_curso (&catedra->nombre_curso[i]);
	}
}
...
void cargar_curso (curso_t * curso)
{
	...
	scanf ("%hhd", &curso->cant_alumnos);
	curso->alumnos = (alumno_t *) malloc ((curso->cant_alumnos) * sizeof (alumno_t));
...
void print_catedra (catedra_t * catedra)
{
	...
	{
		printf ("Curso: \n");
		print_curso (&catedra->nombre_curso[i]);
	}
}
...
void free_catedra (catedra_t * catedra)
{
	...
	for (int i = 0;i < (catedra->cant_cursos);i++)
	{
		free_curso (&catedra->nombre_curso[i]);
	}
